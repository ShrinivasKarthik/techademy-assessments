...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_assessment_shares_token_active 
ON assessment_shares (share_token, is_active) 
WHERE is_active = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_assessments_id_status 
ON assessments (id, status) 
WHERE status = 'published';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_questions_assessment_active 
ON questions (assessment_id, is_active, order_index) 
WHERE is_active = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_assessment_instances_share_token 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_assessment_shares_covering 
