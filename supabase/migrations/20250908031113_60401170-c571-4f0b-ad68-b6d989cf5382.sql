-- Drop and recreate the find_or_create_anonymous_instance function to fix attempt logic
DROP FUNCTION IF EXISTS public.find_or_create_anonymous_instance(uuid, text, text, text, integer);
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- If insertion fails for any reason, return error
    RETURN QUERY SELECT 
      NULL::json,
      false,
      max_attempts_allowed - completed_attempts_count,
      'Failed to create assessment instance. Please try again.'::text;
  END;
END;
$function$;