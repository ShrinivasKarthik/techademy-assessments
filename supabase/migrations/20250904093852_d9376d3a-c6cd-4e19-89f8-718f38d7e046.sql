-- Fix identity creation with proper provider_id
DO $$
DECLARE
...
BEGIN
    ...
    -- Generate identity id that will serve as provider_id too
    ...
    IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = profile_user_id) THEN
        ...
        INSERT INTO auth.identities (
            ...
            profile_user_id::text,  -- Use user_id as provider_id for email provider
            profile_user_id,
        ...
        RAISE NOTICE 'Successfully created auth user for %', profile_email;
    ELSE
        RAISE NOTICE 'User already exists for %', profile_email;
    END IF;
END $$;