-- Add specific policy for anonymous users creating assessment instances via share tokens

CREATE POLICY "Anonymous users can create assessment instances via share tokens" 
ON public.assessment_instances 
FOR INSERT 
WITH CHECK (
  -- Allow anonymous users to create instances when:
