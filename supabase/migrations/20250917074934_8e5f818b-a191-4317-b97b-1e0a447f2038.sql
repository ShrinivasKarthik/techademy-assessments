...
CREATE POLICY "Users can view project files for accessible questions" 
ON project_files 
FOR SELECT 
USING (
  EXISTS (
    ...
    AND (q.created_by = auth.uid() OR q.is_public = true)
  )
);
...
-- Add technology field to questions config
-- This will store free-text technology input instead of fixed languages
-- Example: "React TypeScript with Tailwind CSS", "Django REST API with PostgreSQL"
